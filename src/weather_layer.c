...
void weather_layer_init(WeatherLayer* weather_layer, GPoint pos) {
    ...
    text_layer_init(&weather_layer->temp_layer, GRect(50, 0, 90, 90));
    text_layer_set_text_alignment(&weather_layer->temp_layer, GTextAlignmentCenter);
    text_layer_set_font(&weather_layer->temp_layer, fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ROBOTO_CONDENSED_LIGHT_40)));
...
void weather_layer_set_icon(WeatherLayer* weather_layer, WeatherIcon icon) {
    ...
    layer_set_frame(&weather_layer->icon_layer.layer.layer, GRect(10, 0, 50,50));
...
void weather_layer_set_temperature(WeatherLayer* weather_layer, int16_t t) {
    ...
    // memcpy(weather_layer->temp_str, itoa(-44), 4); // test 3 digit temperature
